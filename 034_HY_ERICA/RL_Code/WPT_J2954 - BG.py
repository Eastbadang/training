...
class NN:
    ...
    def build_nn(self):
        ...
        hidden1 = Dense(units=50, activation=tf.nn.tanh, use_bias=False, kernel_initializer=init.GlorotUniform())(
            input_)
        hidden1 = Dense(units=50, activation=tf.nn.tanh, use_bias=False, kernel_initializer=init.GlorotUniform())(
            hidden1)
...
class Agent:
    ...
    def set_action(self):
        ...
        self.done = False
        self.epsilon = 3 / ((step / 11.1) + 3)  # BG 버전 epsilon(1 -> 0.1 in 300epi)
        # max(1 - (step / num_episodes), 0.1)
        self.exploit_idx = 0
        self.argmax.clear()

        if np.random.rand(1) < self.epsilon:
            print("Exploration")
            choose_explore = list(np.random.choice(24, self.cube_select, replace=False))
            choose_explore.sort()
            ...
            print("Exploitation")
            while not self.done:
                pred = self.nn1.predict(self.greedy_state)[0]
                for j in range(self.cube_select):
                    pred[list(np.where(np.array(self.state) == 1)[0])[j]] = 10
                select = self.rargmax(pred)
                self.next_state[select] = 1
                if int(sum(self.next_state)) == self.cube_select:
                    self.done = True
...
# ####### 정보 문단 #######
print("J2954_BG버전_24C12_20230117")
...
print("Epsilon: 3 / ((step / 11.1) + 3)")

print("Activation Function: tanh, linear / Initializer: GlorotUniform")

print("Layer Configuration: 24 - 25 - 25 - 24, 2 hidden layers")

print("etc: None")
# #########################

agent.state, agent.next_stat, agent.greedy_state = reset()
...
for step in range(num_episodes):
    ...
    nn_output = NNet.predict(agent.state)[0]
    for i in range(agent.cube_select):
        nn_output[actions[-1][i]] = reward

    cost, mae = NNet.update(agent.state, nn_output)
    ...
    print("Best Action, Reward:", Action_list[max_reward_idx], "/ Normal:", Result_list[max_reward_idx][0],
...
nn_output_final = NNet.predict(agent.greedy_state)[0]
