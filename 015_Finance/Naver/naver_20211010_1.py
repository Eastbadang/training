...
for page in range(1, 36):
    # 웹 크롤링 처리
    base_url = f"https://finance.naver.com/sise/field_submit.nhn?menu=market_sum&returnUrl=http://finance.naver.com/sise/sise_market_sum.nhn?page={page}&fieldIds=quant&fieldIds=ask_buy&fieldIds=market_sum&fieldIds=per&fieldIds=ask_sell&fieldIds=roe"
    ...
    for data in stock_datas:
        try:
            ...
            if stock_per == 'N/A':
                ...
                sheet.cell(row=i, column=6).value = float(stock_per.replace(',', ''))
            ...
            if stock_roe == 'N/A':
                ...
                sheet.cell(row=i, column=7).value = float(stock_roe.replace(',', ''))
